...
def create_annotation_file(root_dir, data_file_path, output_dir):
    ...
    for subfolder in subfolders:
        ...
        all_videos_dirs.extend([os.path.join(root_dir, subfolder, x) for x in subsubfolders if
                                os.path.isdir(os.path.join(root_dir, subfolder, x))])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--annot_dir', type=str, default='/home/mvu/Documents/datasets/mixed/afew-va',
                        help='annotation dir')
    parser.add_argument("--afewva_dir", type=str, default='/home/mvu/Documents/datasets/afew-va')

    args = parser.parse_args()

    data_file_path = os.path.join(args.annot_dir, 'annotations.pkl')
    image_dir = os.path.join(args.annot_dir, 'images')
    if not os.path.exists(data_file_path):
        create_annotation_file(args.afewva_dir, data_file_path, image_dir)
    data_file = pickle.load(open(data_file_path, 'rb'))
    all_samples_arousal = np.concatenate(
        [data_file['Training_Set'][x]['arousal'] for x in data_file['Training_Set'].keys()] +
        [data_file['Validation_Set'][x]['arousal'] for x in data_file['Validation_Set'].keys()], axis=0)

    all_samples_valence = np.concatenate(
        [data_file['Training_Set'][x]['valence'] for x in data_file['Training_Set'].keys()] +
